# 👋 Hi, I'm CyberSecOm

🎓 Cybersecurity Enthusiast | Aspiring Blue Team / SOC Analyst

---

## 🔐 About Me
I am a cybersecurity Enthusiast focused on building strong fundamentals in defensive
security. I am currently learning networking, Linux, and web security while gaining
hands-on experience through labs and small security-related projects.

I believe in disciplined learning, ethical practice, and documenting my progress
as I grow in this field.

---

## 🎯 What I'm Currently Working On
- Network & Linux fundamentals
- Web security basics (OWASP Top 10)
- TryHackMe labs and write-ups
- Python & Bash for basic security automation

---

## 🛠 Tools & Technologies
- Operating Systems: Linux (Ubuntu, Kali), Windows
- Security Tools: Nmap, Wireshark, Burp Suite
- Languages: Python (beginner), Bash (basic)
- Platforms: TryHackMe, GitHub

---

## 📂 Featured Repositories
- 📘 **Cybersecurity Learning Journal** – Daily learning logs and reflections
- 🧪 **SOC & Blue Team Labs** – Hands-on labs and walkthroughs
- 🛠 **Small Security Scripts** – Beginner automation and security tools
- 🧠 **Security Notes & Cheat Sheets** – Concepts explained in my own words

---

## 📈 Learning Philosophy
Learn → Practice → Document → Improve

---

## 📫 Connect
- LinkedIn: https://www.linkedin.com/in/omm2011/
